...
async function testCapabilitiesContract() {
  try {
    ...
    const expectedProviders = ['ollama-local', 'llama-cpp-local', 'ollama-cloud', 'nvidia', 'openrouter', 'xiaomimimo', 'openai'];
    if (
      res.status === 200 &&
...
async function testModelCatalogContract() {
  try {
    ...
    if (res.status === 200 && providers.join(',') === 'ollama-local,llama-cpp-local,ollama-cloud,nvidia,openrouter,xiaomimimo,openai' && selected) {
