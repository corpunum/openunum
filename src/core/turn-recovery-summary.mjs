...
export function extractRequirements(userMessage = '') {
  ...
  return {
    ...
    asksStatus: /status|health|inspect|diagnose|report|what happened|why failed|why is|summarize|executions|tool calls|tool calls executed|actions executed/.test(prompt),
    asksExplanation: /\bhow is\b|\bwhat is\b|\bhow does\b|\bexplain\b|\btell me\b|\bunderstand\b|\bworking\b/.test(prompt),
    asksReview: (/\b(check|review|audit|inspect|read|summarize)\b/.test(prompt) && /\b(code|docs?|documentation|changelog|onboard|onboarding|linked|used|memory|mission(?:s)?|provider(?:s)?|routing|skill(?:s)?|tool(?:s)?|executions)\b/.test(prompt)) || /\bmake sense\b|\bmiss something\b|\bnot linked to code\b|\bunused\b/.test(prompt),
