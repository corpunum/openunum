...
import { listBuiltinSkills, resolveBuiltinSkill } from './builtin.mjs';
...
export class SkillManager {
  ...
  listSkills() {
    ...
    const managed = manifest.skills.map((s) => ({
      name: s.name,
    ...
    const merged = new Map(managed.map((row) => [row.name, row]));
  ...
  async executeSkill(name, args = {}) {
    ...
    const manifest = this.loadManifest();
    const skill = manifest.skills.find((s) => s.name === normalized);
    const builtin = skill ? null : resolveBuiltinSkill(normalized);
    if (!skill && !builtin) throw new Error('skill_not_found');
    if (skill && !skill.approved && skill.verdict !== 'safe') throw new Error('skill_not_approved');

    let execPath = skill ? skill.filePath : builtin.execPath;
    if (skill?.isBundle) {
    ...
    return { ok: true, name: builtin.name, result };
  }
